...
* **Upstream Compatibility:** OpenAI-compatible REST endpoints (Configured for Google AI Studio)
...
```
*(Note: Redis is automatically configured internally via Docker Compose).*

### 2. Launch the Stack
Spin up the Gateway, Redis, Prometheus, and Grafana containers in detached mode:
```bash
...
The repository includes dedicated test scripts to verify the core gateway functionality. Make sure you have installed the required Python dependencies in your local environment before running.
